...
- `docs/seo-standards.md` — **mandatory** SEO/AI-discoverability standard for every page (canonical URLs, OG/Twitter tags, schema.org JSON-LD, sitemap/robots.txt/llms.txt). Referenced from [`CLAUDE.md`](CLAUDE.md).
...
- [x] First affiliate link live — Klook, tracked, on Spontaneous Escapes monthly posts only. Bank card referrals (Citi) and expanding to `/airlines/` not done.
- [x] SEO/AI-discoverability standard — see [`docs/seo-standards.md`](docs/seo-standards.md), mandatory per [`CLAUDE.md`](CLAUDE.md). Site retrofitted: canonical URLs, OG/Twitter Card tags, schema.org JSON-LD (Organization, BreadcrumbList, Article per page type), `sitemap.xml`, `robots.txt`, `llms.txt` — all generated by `site/scripts/build.py`, live.
